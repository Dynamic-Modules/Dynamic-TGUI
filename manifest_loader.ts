...
export type ModularTguiScanRoot = {
	/**
	 * Directory to scan, relative to the Dynamic TGUI module root unless absolute.
	 */
	path: string;
...
export function loadModularTguiDefinitions(
	...
	for (const manifestPath of baseManifest.manifestFiles ?? []) {
		const manifest = loadManifestFile(path.resolve(manifestPath));
		const sourceRoot = path.dirname(manifestPath);
		const loadedOverrides = toLoadedOverrides(
			manifest.overrides ?? [],
			sourceRoot,
